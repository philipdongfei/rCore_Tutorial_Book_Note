...
pub struct SDCardCSD {
    ...
    pub ManDeflECC: u8,     /* Manufacture default ECC */
...
pub struct SDCardCID {
    pub ManufactureID: u8,  /* ManufactureID */
...
impl</*'a,*/ X: SPI> SDCard</*'a,*/ X> {
     ...
     * @param None
     * @retval The SD Response info if succeeded, otherwise Err
     */
    ...
    pub fn read_sector(&self, data_buf: &mut [u8], sector: u32) -> Result<(), ()> {
        ...
        /* Check if the SD acknowledged the read block command: R1 response (0x00: no
         * errors) */
        if self.get_response() != 0x00 {
            self.end_cmd();
            return Err(());
...
/** CS value passed to SPI controller, this is a dummy value as SPI0_CS3 is not mapping to
 * anything 
 * in the FPIOA */
const SD_CS: u32 = 3;
